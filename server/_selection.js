...
async function selectVideos(store,input,text=generateText){
  ...
  library=library.filter(a=>a.kind==='video'&&!a.archived);
  ...
  const key='legado-studio/selection/'+createHash('sha256').update(JSON.stringify({v:1,story:input.story,options})).digest('hex')+'.json';
  ...
  return scenes.map(s=>{const c=choices.find(c=>c.scene===s.scene);return {...s,videoOnly:true,assetId:c.assetId||'',reason:c.reason,alternatives:options.find(o=>o.scene===s.scene).candidates.map(a=>a.id)};});
}
