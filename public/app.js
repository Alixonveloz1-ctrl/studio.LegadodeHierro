...
async function generate(){
  ...
  // IDENTIDAD BASE DEL PERSONAJE — compartida por todos los modos
  var identidadBase='CHARACTER IDENTITY (mandatory): A handsome 35-year-old Hispanic man, short black hair slicked back, short dark beard, strong jawline, intense dark brown eyes, serious expression never smiling. American 2D comic book illustration style, clean ink lines, cel-shading, NOT photorealistic, NOT 3D render. 9:16 vertical portrait format. No text overlays in image. NO robots, NO futurism, NO sci-fi, NO broken chains, NO magic effects. Real business and financial world only. NO rain near character. GAZE: character looks at whatever the action requires. Does NOT look at camera unless prompt says so.';
  var arquetiposVirales='VIRAL SCENE ARCHETYPES (use these, rotate between them):\nTRABAJADOR: plain dark green t-shirt, humble workshop or basic desk, old laptop, coins or small bills on worn wooden table. Viewer recognizes himself here.\nCRISIS: multiple screens red charts FALLING/GLOBAL DOWNTURN; OR man holding tablet stamped CERRADO facing rain-streaked window city night.\nOBJETO SIMBOLICO: extreme close-up luxury watch wrist finger tapping crystal; OR keyboard screen ENVIO CAPITAL GLOBAL CONFIRMADO green monospace; OR stacked banded bills coins beside laptop.\nHERRAMIENTA: holographic panel IA MKT/VENTAS neural brain; OR watchmaker precision magnifying lens; OR green-ascending multi-monitor trading terminal.\nALPHA: dark three-piece suit walking toward camera glass bridge skyscrapers dawn; OR back-view glass corporate hallway city night; OR rooftop balcony golden hour overlooking city.';
  var identidad=identidadBase+'\n\n'+arquetiposVirales;
  var msg;
  if(sMode==='impacto'){
    msg=SP+'\n\n---\n\nGenera un episodio COMPLETO:\nPILAR: '+(tO?tO.label+' - '+tO.desc:'Independencia Financiera')+'\nGANCHO: '+(hO?hO.label:'Dato Crudo')+' - '+(hi[sH]||hi.dato)+'\nCONCEPTO: '+topic+'\n\n'+identidad+'\n\nRecuerda: BLOQUE A es solo texto hablado sin prompts (maximo 55 palabras). BLOQUE C son los 2 prompts de imagen. BLOQUE F es el guion en ingles sin prompts.';
  } else {
    msg=SP+'\n\n---\n\nGenera un episodio COMPLETO:\nPILAR: '+(tO?tO.label+' - '+tO.desc:'Independencia Financiera')+'\nDURACION: '+(dO?dO.label:'60 segundos')+'\nGANCHO: '+(hO?hO.label:'Dato Crudo')+' - '+(hi[sH]||hi.dato)+'\nCONCEPTO: '+topic+'\n\n'+identidad+(sMode===' historia'?'\n\nMODO HISTORIA: BLOQUE C sigue arco TRABAJADOR(1) → CRISIS(2) → HERRAMIENTA(3) → OBJETO(4) → ALPHA(5).':'')+'\n\nRecuerda: BLOQUE A es solo texto hablado sin prompts. BLOQUE C son los 5 prompts de imagen. BLOQUE F es el guion en ingles sin prompts.';
  }
  try{
