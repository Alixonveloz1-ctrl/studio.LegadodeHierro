...
async function studioAPI(action,data,endpoint){
  var r=await fetch(endpoint||'/api/studio',{method:'POST',headers:{'Content-Type':'application/json'},signal:AbortSignal.timeout((endpoint==='/api/studio-job'||endpoint==='/api/trends')?310000:57000),body:JSON.stringify(Object.assign({action:action},data||{}))});
...
async function studioLibraryLinks(rows){
  ...
  for(var i=0;i<missing.length;i+=100){
    ...
    (d.items||[]).forEach(function(a){STUDIO_LIBRARY_LINKS[a.id]={url:a.url,asset:a,expires:Date.now()+300000};});
  }
...
async function studioBuildPlan(){
  ...
  studioMessage('Comprobando qué videos cuentan esta historia y mantienen la continuidad…');
  var chosen=await studioAPI('select-videos',{story:owner.a,scenes:scenes});if(owner!==lastRes)return;
  if(!Array.isArray(chosen.plan)||chosen.plan.length!==scenes.length)throw new Error('No se pudo completar la selección de videos.');
  STUDIO.plan=chosen.plan;studioPaintPlan();await studioPersistAssets();updUnifyCard();
  var gaps=STUDIO.plan.filter(function(p){return !studioShotVideo(p);}).length;
  studioMessage(gaps?'Plan preparado: '+gaps+' tomas no encontraron un video compatible. Puedes buscar otro clip; no se generó contenido nuevo.':STUDIO.plan.length+' tomas elegidas. Revisa la secuencia antes de montar; no se generó contenido nuevo.');
}
...
function studioPreviewError(plan,grid,index,id){
  ...
  var row=grid.querySelector('[data-shot="'+index+'"]'),box=row&&row.querySelector('.studio-plan-preview');if(!box)return;
  box.textContent='No se pudo cargar esta toma asignada.';
  var retry=document.createElement('button');retry.textContent='Recargar vista previa';retry.onclick=function(){delete STUDIO_LIBRARY_LINKS[id];studioPlanPreviews(plan,grid);};box.appendChild(retry);
}
async function studioPlanPreviews(plan,grid){
  try{
    ...
    var links=await studioLibraryLinks(ids.map(function(id){return {id:id};}));
    if(STUDIO.plan!==plan)return;
  ...
  }catch(e){plan.forEach(function(p,i){if(p.assetId)studioPreviewError(plan,grid,i,p.assetId);});}
}
...
async function studioPrepareMontage(audio){
  ...
  if(plan.length){
    ...
    for(var i=0;i<scenes.length;i++){
      ...
      else if(im&&im.src)throw new Error('Convierte la imagen de la escena '+(sc.scene+1)+' a video antes de montar.');
