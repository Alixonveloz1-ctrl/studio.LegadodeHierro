...
(async () => {
  ...
  t('pide UNA sola figura, explícitamente',
    /ONE person only/.test(l.prompt) && /NOT a collage/.test(l.prompt));
  t('manda el encuadre, para que la cara no salga diminuta',
    /at least 55% of the image height/.test(l.prompt));
  ...
  // Se guarda la vista 4 sin haber hecho la 3: el hueco 3 tiene que quedar vacío.
  r = await llamar({ action: 'guardar', personaje: ins, vistas: [{ i: 3, b64: GENERADA }] });
  ins = r.body.personaje;
  t('el hueco de la vista que falta se queda vacío', ins.refs[2] === null, JSON.stringify(ins.refs));
  t('la vista 4 se queda en SU sitio, no se corre al hueco libre',
    ins.refs[3] === 'personajes/insignia/vista-4.png', ins.refs[3]);

  r = await llamar({ action: 'imagenes', id: 'insignia' });
  t('al pintarlas, se dice qué vista es cada una',
    JSON.stringify(r.body.indices) === '[0,1,3]', JSON.stringify(r.body.indices));
  t('y solo vienen las que existen', r.body.refs.length === 3);
  ...
  t('las vistas buenas que ya había no se pierden',
    ins.refs[0] === 'personajes/insignia/vista-1.png' && ins.refs[3] === 'personajes/insignia/vista-4.png');
