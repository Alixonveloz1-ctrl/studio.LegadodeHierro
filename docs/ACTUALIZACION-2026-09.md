...
La actualización del código todavía necesita aplicarse a los servicios reales. El estado del último commit en GitHub identifica el proyecto existente de Vercel como `legado-de-hierro-final`. Durante esta intervención, la conexión de Vercel respondió `Unknown tool` al consultar equipos y proyectos. Es un fallo técnico de la conexión, no falta de autorización del propietario. No se ha confirmado un despliegue de esta actualización ni se han ejecutado generaciones reales de texto, voz o imágenes para validarla.
...
4. Despliega esta misma revisión del repositorio en el **proyecto existente de Vercel**, conservando las variables de entorno. Es una actualización coordinada: el contrato de montaje cambió.
...
El job dispone de una hora por intento, un reintento y 2 GiB de memoria. Su cuenta de servicio necesita acceso al bucket. Si el despliegue del job falla, completa esa configuración antes de activar el nuevo montaje en Vercel. No se crea otro sistema de usuarios ni se migran o eliminan los archivos originales.

## Verificación realizada y pendiente

Pasaron 32 pruebas automatizadas de guiones por partes, revisión y corrección editorial, recuperación de audio, escrituras concurrentes, API, catálogo, mediciones, controles DOM y arranque del ejecutor. Se comprobó que una revisión guardada se recupera sin otra llamada, que se rechazan citas inexistentes y que la corrección no entra en un ciclo infinito. FFmpeg produjo un MP4 vertical real de ocho minutos con narración sintética dividida, imágenes, video, música audible cerca del final y subtítulos. La diferencia entre duración del video y la narración quedó dentro de un fotograma en las comprobaciones del plan.
